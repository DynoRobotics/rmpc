...
fn row_multiplication<const R: usize, const C: usize>(
...
fn row_addition<const R: usize, const C: usize>(
    matrix: &mut Matrix<R, C>,
    source: usize,
    dest: usize,
    scale: f64,
...
///
/// ```
/// # use mpc::math::{Matrix, inv_no_pivot};
...
/// assert!(zero.0.iter().flatten().all(|elem| elem.abs() <= 1e-10));
/// ```
pub fn inv_no_pivot<const N: usize>(matrix: Matrix<N, N>) -> Matrix<N, N> {
    let mut lhs = matrix;
